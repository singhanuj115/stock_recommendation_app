...
class TradeManager:
    ...
    def run():
        ...
        TradeManager.intraday_trades_dir = os.path.join(
            trades_dir, Utils.get_today_date_str())
        if not os.path.exists(TradeManager.intraday_trades_dir):
            logging.info('TradeManager: Intraday Trades Directory %s does not exist. Hence going to create.',
                         TradeManager.intraday_trades_dir)
        ...
        """
        Append to ticker object of trade-manager
        The ticker listener object of trade-manager
        by register listener.
        """
    ...
    def register_strategy(strategy_instance):
        TradeManager.strategy_to_instance_map[strategy_instance.get_name(
        )] = strategy_instance

    @staticmethod
    def load_all_trades_from_file():
        trades_filepath = os.path.join(
            TradeManager.intraday_trades_dir, 'trades.json')
        if not os.path.exists(trades_filepath):
            logging.warning(
                'TradeManager: loadAllTradesFromFile() Trades Filepath %s does not exist', trades_filepath)
            return
        TradeManager.trades = []
    ...
    def add_new_trade(trade):
        ...
        TradeManager.trades.append(trade)
        logging.info(
            'TradeManager: trade %s added successfully to the list', trade.trade_id)
    ...
    def disable_trade(trade, reason):
        if trade is not None:
            logging.info(
                'TradeManager: Going to disable trade ID %s with the reason %s', trade.trade_id, reason)
            trade.trade_state = TradeState.DISABLED

    @staticmethod
    def ticker_listener(tick):
        # logging.info('tickerLister: new tick received for %s = %f', tick.trading_symbol, tick.last_traded_price);
        ...
        for strategy in TradeManager.strategy_to_instance_map:
            long_trade = TradeManager.get_untriggered_trade(
                tick.trading_symbol, strategy, Direction.LONG)
            short_trade = TradeManager.get_untriggered_trade(
                tick.trading_symbol, strategy, Direction.SHORT)
            if long_trade is None and short_trade is None:
                continue
            strategy_instance = TradeManager.strategy_to_instance_map[strategy]
            if long_trade is not None:
    ...
    def execute_trade(trade):
        ...
        oip.order_type = OrderType.MARKET if trade.place_market_order else OrderType.LIMIT
        ...
        try:
            ...
            logging.error(
                'TradeManager: Execute trade failed for trade_id %s: Error => %s', trade.trade_id, str(e))
            return False

        logging.info(
            'TradeManager: Execute trade successful for %s and entryOrder %s', trade, trade.entry_order)
        return True
    ...
    def fetch_and_update_all_trade_orders():
        ...
        for trade in TradeManager.trades:
            ...
            if trade.target_order is not None:
    ...
    def track_and_update_all_trades():
        for trade in TradeManager.trades:
            if trade.trade_state == TradeState.ACTIVE:
                ...
                if trade.intraday_square_off_timestamp is not None:
                    now_epoch = Utils.get_epoch()
                    if now_epoch >= trade.intraday_square_off_timestamp:
                        TradeManager.square_off_trade(
    ...
    def track_sl_order(trade):
        ...
        if trade.sl_order is None:
            ...
            if trade.sl_order.order_status == OrderStatus.COMPLETE:
                ...
                logging.error(
                    'SL order %s for trade_id %s cancelled outside of Algo. Setting the trade as completed with exit price as current market price.',
                    trade.sl_order.order_id, trade.trade_id)
                exit = TradeManager.symbol_to_cmp_map[trade.trading_symbol]
                TradeManager.set_trade_to_completed(
    ...
    def check_and_update_trail_sl(trade):
        ...
        if update_sl:
            ...
            try:
                ...
                logging.info('TradeManager: Trail SL: Successfully modified stopLoss from %f to %f for trade_id %s',
                             old_sl, new_trail_sl, trade.trade_id)
                ...
                logging.error('TradeManager: Failed to modify SL order for tradeID %s orderId %s: Error => %s',
                              trade.trade_id, trade.sl_order.order_id, str(e))
        ...
        if trade.target_order is None:
            ...
            if trade.target_order.order_status == OrderStatus.COMPLETE:
                ...
                logging.error(
                    ...
                    trade.target_order.order_id, trade.trade_id)
                exit = TradeManager.symbol_to_cmp_map[trade.tradingSymbol]
                TradeManager.set_trade_to_completed(
    ...
    def place_sl_order(trade):
        ...
        try:
            ...
            logging.error(
                'TradeManager: Failed to place SL order for tradeID %s: Error => %s', trade.trade_id, str(e))
            return False
        logging.info('TradeManager: Successfully placed SL order %s for tradeID %s', trade.sl_order.order_id,
                     trade.trade_id)
        return True
    ...
    def place_target_order(trade, is_market_order=False):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to place Target order for tradeID %s: Error => %s', trade.trade_id,
                          str(e))
            return False
        logging.info('TradeManager: Successfully placed Target order %s for tradeID %s', trade.target_order.order_id,
                     trade.trade_id)
        return True
    ...
    def cancel_entry_order(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to cancel Entry order %s for tradeID %s: Error => %s',
                          trade.entry_order.order_id, trade.trade_id, str(e))
        logging.info('TradeManager: Successfully cancelled Entry order %s for tradeID %s', trade.entry_order.order_id,
                     trade.trade_id)
    ...
    def cancel_sl_order(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to cancel SL order %s for tradeID %s: Error => %s',
                          trade.sl_order.order_id, trade.trade_id, str(e))
        logging.info('TradeManager: Successfully cancelled SL order %s for tradeID %s', trade.sl_order.order_id,
                     trade.trade_id)
    ...
    def cancel_target_order(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to cancel Target order %s for tradeID %s: Error => %s',
                          trade.target_order.order_id, trade.trade_id, str(e))
        logging.info('TradeManager: Successfully cancelled Target order %s for tradeID %s', trade.target_order.order_id,
                     trade.trade_id)

    @staticmethod
    def set_trade_to_completed(trade, exit, exit_reason=None):
        trade.trade_state = TradeState.COMPLETED
        trade.exit = exit
        trade.exit_reason = exit_reason if trade.exit_reason is None else trade.exit_reason
        ...
        logging.info(
            ...
            trade.strategy, trade.trading_symbol, trade.filled_qty, trade.entry, trade.exit, trade.pnl,
            trade.exit_reason)

    @staticmethod
    def square_off_trade(trade, reason=TradeExitReason.SQUARE_OFF):
        logging.info(
            'TradeManager: squareOffTrade called for tradeID %s with reason %s', trade.trade_id, reason)
        if trade is None or trade.trade_state != TradeState.ACTIVE:
            return
        ...
        if trade.target_order is not None:
            ...
            logging.info(
                'TradeManager: placing new target order to exit position for tradeID %s', trade.trade_id)
    ...
    def convert_json_to_trade(json_data):
        ...
        trade.entry_order = TradeManager.convert_json_to_order(
            json_data['entry_order'])
        trade.sl_order = TradeManager.convert_json_to_order(
            json_data['sl_order'])
        trade.target_order = TradeManager.convert_json_to_order(
            json_data['target_order'])
        return trade
