...
class Trade:
    def __init__(self, trading_symbol=None):
        ...
        self.intraday_square_off_timestamp = None
        ...
        self.create_timestamp = Utils.get_epoch()
        # Timestamp when the trade gets triggered and order placed
    ...
    def __str__(self):
        return "ID=" + str(self.trade_id) + ", state=" + self.trade_state + ", symbol=" + self.trading_symbol \
               ...
               + ", productType=" + self.product_type + ", reqEntry=" + str(self.requested_entry) \
               + ", stopLoss=" + str(self.stop_loss) + ", target=" + str(self.target) \
