...
class SampleStrategy(BaseStrategy):
    ...
    def get_instance():  # singleton class
        if SampleStrategy.__instance is None:
            SampleStrategy()
        return SampleStrategy.__instance

    def __init__(self):
        if SampleStrategy.__instance is not None:
    ...
    def process(self):
        ...
        for symbol in self.symbols:
            ...
            if quote is None:
                ...
                continue
            long_breakout_price = Utils.round_to_nse_price(
                quote.close + quote.close * 0.5 / 100)
            short_breakout_price = Utils.round_to_nse_price(
                quote.close - quote.close * 0.5 / 100)
            cmp = quote.last_traded_price
            logging.info('%s: %s => long = %f, short = %f, CMP = %f', self.get_name(), symbol, long_breakout_price,
                         short_breakout_price, cmp)
    ...
    def generate_trade(self, trading_symbol, direction, breakout_price, cmp):
        ...
        if direction == 'LONG':
            trade.stop_loss = Utils.round_to_nse_price(
                breakout_price - breakout_price * self.sl_percentage / 100)
        ...
        if direction == 'LONG':
            trade.target = Utils.round_to_nse_price(
                breakout_price + breakout_price * self.target_percentage / 100)
        else:
            trade.target = Utils.round_to_nse_price(
                breakout_price - breakout_price * self.target_percentage / 100)
