...
class BaseStrategy:
    def __init__(self, name):
        ...
        self.start_timestamp = Utils.get_market_start_time()
        ...
        # Applicable if is_fno is True (Set means 1CE/1PE or 2CE/2PE etc based on your strategy logic)
    ...
    def calculate_capital_per_trade(self):
        ...
        capital_per_trade = int(
            self.capital * leverage / self.max_trades_per_day)
    ...
    def run(self):
        ...
        if not self.enabled:
            logging.warning(
                "%s: Not going to run strategy as its not enabled.", self.get_name())
            return

        if Utils.is_market_closed_for_the_day():
            logging.warning(
                "%s: Not going to run strategy as market is closed.", self.get_name())
            return
        ...
        if not self.can_trade_today():
            logging.warning(
                "%s: Not going to run strategy as it cannot be traded today.", self.get_name())
            return

        now = datetime.now()
        if now < self.start_timestamp:
            wait_seconds = Utils.get_epoch(
                self.start_timestamp) - Utils.get_epoch(now)
            logging.info("%s: Waiting for %d seconds till startegy start timestamp reaches...", self.get_name(),
                         wait_seconds)
            if wait_seconds > 0:
        ...
        while True:
            if Utils.is_market_closed_for_the_day():
                logging.warning(
                    "%s: Exiting the strategy as market closed.", self.get_name())
                break
    ...
    def should_place_trade(self, trade, tick):
        ...
        num_of_trades_placed = TradeManager.get_number_of_trades_placed_by_strategy(
            self.get_name())
